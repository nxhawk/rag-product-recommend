...
**Lưu ý về "thinking" của Gemini.** `gemini-2.5-flash`/`flash-lite` mặc định suy luận nội bộ ("thinking"), và các token thinking này bị trừ vào **cùng** ngân sách `max_output_tokens` với câu trả lời cuối cùng. Nếu không kiểm soát, thinking có thể chiếm hết ngân sách trước khi model kịp viết JSON, tạo ra response rỗng/cắt cụt khiến validate schema thất bại ở gần như mọi lần gọi — đây chính là nguyên nhân gốc của một lỗi trước đây khiến `/api/recommend` luôn rơi về fallback. `GeminiProvider.generate()` (`src/generation/llm_client.py`) giờ set `thinking_config=ThinkingConfig(thinking_budget=0)` cho các model flash/flash-lite để ngăn việc này. `gemini-2.5-pro` không hỗ trợ tắt thinking, nên được giữ nguyên — biện pháp giảm thiểu duy nhất với pro là đặt `max_output_tokens` đủ rộng rãi.

**Nguồn:** `src/guardrails/output/schemas.py`, `src/guardrails/output/validator.py`, `src/generation/llm_client.py`

### 3b. Grounding

Ngay cả khi JSON đúng định dạng, LLM vẫn có thể "bịa" ra một tên sản phẩm chưa từng được truy xuất. `ground_recommendations()` / `ground_compare_analysis()` so khớp `name` của từng item với danh sách sản phẩm đã truy xuất/so sánh và loại bỏ những gì không khớp, thêm một mục vào `warnings` ghi rõ số lượng đã loại. Nếu *toàn bộ* item đều bị loại, pipeline xử lý giống như một lỗi schema và fallback.

Việc so khớp không phân biệt hoa/thường, khoảng trắng, và có dung sai cho việc diễn đạt lại nhẹ — không phải so sánh `==` thuần túy: prompt yêu cầu LLM copy `name` nguyên văn, nhưng model vẫn thường xuyên bỏ bớt tiền tố/hậu tố (ví dụ "Điện thoại " ở đầu hoặc "| Chính hãng VN/A" ở cuối) hoặc chỉnh sửa dấu câu. Một tên được coi là "grounded" nếu nó (1) khớp chính xác sau khi normalize, (2) là substring của — hoặc chứa — một tên đã biết, được giới hạn bởi `grounding_min_containment_chars` để một mảnh ngắn (ví dụ chỉ tên hãng) không thể khớp với mọi thứ, hoặc (3) khớp gần theo `difflib.SequenceMatcher` với tỉ lệ ≥ `grounding_fuzzy_ratio`. Thay đổi này khắc phục một lỗi trước đây khi so sánh chỉ-khớp-chính-xác khiến guardrail đầu ra loại bỏ *toàn bộ* gợi ý và fallback ở gần như mọi request.
...
| `grounding_min_containment_chars` | `8` | `output/grounding.py` (độ dài tối thiểu để so khớp substring) |
| `grounding_fuzzy_ratio` | `0.82` | `output/grounding.py` (ngưỡng `difflib.SequenceMatcher`) |
...
| `tests/unit/guardrails/test_output.py` | Validate schema, grounding (kể cả khớp mờ/fuzzy), các hàm dựng fallback |
...
| `tests/unit/generation/test_llm_client_gemini.py` | Cấu hình JSON mode + thinking-budget của `GeminiProvider` (test hồi quy cho lỗi "luôn fallback" do thinking token chiếm hết `max_output_tokens`) |
